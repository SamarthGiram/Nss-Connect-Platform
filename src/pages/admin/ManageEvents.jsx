...
const ManageEvents = () => {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Error banner */}
      {error && (
        <div className="bg-red-50 border border-red-200 rounded-2xl px-5 py-3 text-red-600 text-sm font-semibold flex items-center gap-2">
          ⚠️ {error}
        </div>
      )}
                ...
                <div className="w-14 h-14 bg-white/15 rounded-xl flex flex-col items-center justify-center flex-shrink-0">
                  {d ? (
                    <>
                      ...
                      <p className="text-[9px] font-bold text-[#ef7041] uppercase">{d.toLocaleString('default',{month:'short'})}</p>
                    </>
                  ) : <HiOutlineCalendar size={22} className="text-white/60"/>}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${TAG_COLORS[ev.tag] || 'bg-gray-100 text-gray-500'}`}>{ev.tag}</span>
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${STATUS_COLORS[ev.status]}`}>{ev.status}</span>
                  </div>
                  <h3 className="text-white font-extrabold text-sm leading-tight truncate">{ev.title}</h3>
                </div>
              </div>
              <div className="p-5">
                ...
                <div className="space-y-1.5 mb-4">
                  ...
                  <p className="text-xs text-gray-400 flex items-center gap-1.5 font-medium">
                    <HiOutlineUserGroup size={12} className="text-violet-500"/> {ev.volunteers || 0} volunteers
                  </p>
                </div>
      ...
      {filtered.length === 0 && !loading && (
        <div className="text-center py-16 bg-white rounded-2xl border border-gray-100">
          ...
          <p className="text-gray-400 text-sm mt-1">Create your first event using the button above</p>
        </div>
      )}
    </div>
  );
